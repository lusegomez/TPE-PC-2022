...
struct socks5 * create_new_sock5(int client_fd) {
    ...
    s->stm.states = states_definition;
    s->stm.current = NULL;
    stm_init(&(s->stm));
...
void socksv5_passive_accept(struct selector_key * key) {
    ...
    sock = create_new_sock5(client_sock);
    if(sock == NULL) {
        goto finally;
    }

    if(selector_register(key->s, client_sock, &socks5_active_handler, OP_READ, sock) != SELECTOR_SUCCESS) {
        goto finally;
    }
